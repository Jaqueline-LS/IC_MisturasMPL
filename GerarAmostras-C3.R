...
cores<-c("#c2a5cf","#BAF3DE","#C9E69E","#FF9B95","#FFC29A", "#FF9BB4")
par<-par(pch=19)
...
nome.plot<-"c3_"
# nome.amostra<-"Amostras.corrigidas/c4_"
# nome.plot<-"c4_"
...
# par(mfrow=c(1,3), mar=c(2,2,1,1))
# 
# VerificarEstimacao(amostra, arg.grupos, theta, tipo="Cenário 3")
# 
# dev.off()
...
# -------------------------------Replicações------------------------------------------------
alfas<-c(0.1,0.1)
sizes<-c(100,300,500,1000,2000,5000)

#gera.amostras(5000)
   # Esse comando gera as M amostras de tamanho 300 o arquivo é salvo na pasta Amostras 


# Para mais detalhes da gera  ção das replicas das amostras confeir o arquivo "GerarAmostras_geral.R"
...
jpeg(file=paste0("Resultados/", nome.plot,"curvas2.jpg"), width = 1500, height = 800, quality = 100, pointsize = 20)
...
tabela.final<-cbind.data.frame(tabela.inicial,tabela.100,tabela.300,tabela.500,tabela.1000, tabela.2000, tabela.5000)
...
tabela_latex <- knitr::kable(tabela.final, caption = paste0("C3 - Mal separados"), format = "latex", escape = FALSE, booktabs=T) %>%
  add_header_above(c(" " = 2,"n=100" = 3 ,"n=300" = 3,"n=500" = 3,"n=1000" = 3,"n=2000"=3, "n=5000"=3)) %>%
